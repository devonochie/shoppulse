...
class AuthController {
    ...
    async register (req: express.Request, res: express.Response, _next: express.NextFunction) {
        try {
            const parsed = userValidators.safeParse(req.body)
            if (!parsed.success) {
            ...
            logger.error('Registeration failed: ', error)
        ...
        try {
            ...
            if (refreshToken) {
                try {
                    ...
                    logger.debug('Token cleanup skipped:', error);
                }
            }
